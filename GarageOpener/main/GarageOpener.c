...
#define CONFIG_USE_HTTPS 1

#define API_SECRET API_AUTH_KEY_GARAGE
...
esp_err_t httpClientOTA(esp_http_client_event_t* event);
esp_err_t connectWifi();
esp_err_t initServer();
esp_err_t httpClientOTA(esp_http_client_event_t* event);
esp_err_t initGpio(int pin);
void handleOpenGarage(char* state, int force, int forceTime);
...
esp_err_t connectWifi()
{
  esp_err_t nvsErrCheck
      = nvs_flash_init();
  if (nvsErrCheck == ESP_ERR_NVS_NO_FREE_PAGES || nvsErrCheck == ESP_ERR_NVS_NEW_VERSION_FOUND) {
...
void sendRes(httpd_req_t* req, char* res)
{
  ...
  ESP_ERROR_CHECK_WITHOUT_ABORT(httpd_resp_set_hdr(req, (const char*)"Access-Control-Allow-Origin", (const char*)"*"));
  ESP_ERROR_CHECK_WITHOUT_ABORT(httpd_resp_set_hdr(req, (const char*)"Access-Control-Allow-Headers", (const char*)"*"));
...
esp_err_t handleOPTIONS_garage(httpd_req_t* req)
{
  ...
  ESP_ERROR_CHECK_WITHOUT_ABORT(httpd_resp_set_hdr(req, (const char*)"Access-Control-Allow-Methods", (const char*)"OPTIONS, GET, POST, PUT"));
...
esp_err_t handlePUT_garage(httpd_req_t* req)
{
  ...
  cJSON* authObj = cJSON_GetObjectItemCaseSensitive(reqBodyJSON, "auth");
  ...
  if (strcmp(API_SECRET, auth) != 0) {
    httpd_resp_send_err(req, HTTPD_401_UNAUTHORIZED, "Auth Key Invalid");
    ESP_LOGW(TAG, "Expected: %s", API_SECRET);
    ESP_LOGW(TAG, "Got     : %s", auth);
  }

  handleOpenGarage(NULL, 1, forceTime);

  sendRes(req, "open/closing");
...
esp_err_t handlePOST_garage(httpd_req_t* req)
{
  ...
  char* reqGarageState = cJSON_GetObjectItem(reqBodyJSON, "setState")->valuestring;

  for (int i = 0; reqGarageState[ i ]; i++)
    reqGarageState[ i ] = tolower(reqGarageState[ i ]);

  if ((strcmp(reqGarageState, "open") == 0) || (strcmp(reqGarageState, "close") == 0)) {
    handleOpenGarage(reqGarageState, 0, forceTime);

    sendRes(req, reqGarageState);
...
void handleOpenGarage(char* state, int toggle, int forceTime)
{
  if (toggle) {
    if (forceTime == 0 || forceTime < 30000)
      forceTime = 400;
    ...
    if (strcmp(garageState, "open") == 0) {
      garageState = "close";
    } else if (strcmp(garageState, "close") == 0) {
      garageState = "open";
    }
  } else if (state != NULL) {
